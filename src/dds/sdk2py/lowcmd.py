"""Local SDK2 Python lowcmd subscription boundary between the sidecar and Isaac Sim."""
...
LOGGER = get_logger("dds.sdk2py_lowcmd")


class Sdk2PyLowCmdUdpSubscriber:
    """Receive lowcmd packets from the SDK2 Python sidecar and cache the latest sample."""
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if exc.errno != errno.EADDRINUSE or self._bind_port == 0:
                ...
                raise
            LOGGER.warning(
                "SDK2 Python lowcmd UDP port %s is already in use on %s; "
                "falling back to an ephemeral localhost port.",
                self._bind_port,
                self._bind_host,
            )
        ...
        LOGGER.info("SDK2 Python lowcmd UDP subscriber ready on %s:%s", self._bind_host, self.bound_port)
        return True
    ...
    def _has_complete_fields(
        ...
        if short_fields:
            LOGGER.warning("dropped SDK2 Python `rt/lowcmd` packet with incomplete fields: %s", short_fields)
            return False
        return True
    ...
    def _warn_extra_message_fields(incoming_count: int) -> None:
        LOGGER.warning(
            "received SDK2 Python `rt/lowcmd` with %s motor slots; consuming only the first %s G1 body-joint commands.",
            incoming_count,
            BODY_JOINT_COUNT,
        )
    ...
    def _warn_short_message(incoming_count: int) -> None:
        LOGGER.warning(
            "dropped SDK2 Python `rt/lowcmd` message with %s motor slots; expected at least %s body-joint commands.",
            incoming_count,
            BODY_JOINT_COUNT,
        )
